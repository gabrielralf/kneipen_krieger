  ...
  Future<void> _saveAndPost() async {
    ...
    try {
      ...
      if (user == null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please login to post a suggestion.')),
        );
        return;
      }
      ...
      final bucket = client.storage.from('price-suggestions');
      ...
      await client.from('price_suggestions').insert({
        'user_id': user.id,
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Saved & posted.')));
      setState(() {
        _drinkType = null;
        _price = 5.0;
        _photo = null;
